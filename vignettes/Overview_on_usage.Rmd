...
As an example, we will use the `mtcars` data set.built into R.
...
outputs_raw<-corr_mat(dat=mtcars, raw=T, file1 = here("cor.df.csv"),
           file2 = here("p.df.csv"))
outputs_log<-corr_mat(dat=mtcars, raw=T, file1 = here("cor.df.log.csv"),
           file2 = here("p.df.log.csv"))
outputs_rank<-corr_mat(dat=mtcars, raw=T, file1 = here("cor.df.rank.csv"),
...
cor.df<-outputs[1]#correlation coefficients
p.df<-outputs[2]#p values 
